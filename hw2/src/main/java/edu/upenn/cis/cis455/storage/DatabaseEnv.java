...
public class DatabaseEnv implements StorageInterface {
    ...
    PrimaryIndex<Integer, User> userById;
    SecondaryIndex<String, Integer, User> userByUsername;
    ...
    public DatabaseEnv(String directory) {
        try {
            ...
            SequenceConfig userSequenceConfig = store.getSequenceConfig("docId");
            userSequenceConfig.setCacheSize(1);
            store.setSequenceConfig("userId", userSequenceConfig);
            ...
            userByUsername = store.getSecondaryIndex(userById, String.class, "username");

        } catch (DatabaseException dbe) {
            logger.debug("Error opening environment and store");
            logger.debug(dbe);
        }
    }
    ...
    public void addChannel(String name, String user, String xpath) {
        ...
        Channel channel = new Channel(name, user, xpath);
        channelByName.put(channel);
        txn.commit();

        logger.info("Added channel to channels database: " + name);
    }

    public void addDocToChannel(String channelName, String url) {
        Transaction txn = env.beginTransaction(null, null);
        ...
        logger.info("Added document " + url + " to channel " + channelName);
    }

    public boolean containsChannel(String name) {
        return channelByName.contains(name);
    }

    public List<String> getAllChannelNames() {
        EntityCursor<Channel> channels = channelByName.entities();
        List<String> res = new ArrayList<String>();

        for (Channel channel : channels) {
            res.add(channel.name);
        }
        channels.close();
        return res;
    }
    ...
    public void resetRun() {
        ...
        Transaction txn = env.beginTransaction(null, null);
        EntityCursor<Channel> channels = channelByName.entities(txn, null);
        for (Channel channel : channels) {
            channel.clearDocuments();
    ...
    public int addUser(String username, String password) {
        ...
        Transaction txn = env.beginTransaction(null, null);
        if (userByUsername.contains(username)) {
            txn.abort();
            return -1;
        }
        ...
        logger.info("Added user to users database: " + username);
        return user.id;
    }
    ...
    public boolean getSessionForUser(String username, String password) {
        ...
        String passwordHash = Security.sha256Hash(password);
    ...
    public String toString() {
        ...
        // Print users
        EntityCursor<User> users = userById.entities();
        count = 0;
